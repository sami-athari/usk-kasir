...
<div>
    ...
    <div class="mb-10">
        ...
        <h1 class="text-4xl font-light text-slate-800 dark:text-white tracking-tight">Edit <span class="font-bold">Kategori</span></h1>
        <p class="text-slate-500 dark:text-slate-400 mt-2">Ubah data kategori {{ $category->name }}</p>
    </div>
            ...
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" required
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none">
