...
<div>
    ...
    <div class="mb-10">
        ...
        <h1 class="text-4xl font-light text-slate-800 tracking-tight">Edit <span class="font-bold">Bahan Baku</span></h1>
        <p class="text-slate-500 mt-2">Perbarui data: {{ $ingredient->name }}</p>
    </div>

    <!-- Form Card -->
    <div class="max-w-2xl">
        <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
            <form action="{{ route('admin.ingredients.update', $ingredient->id) }}" method="POST" class="space-y-6">
                @csrf @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="stock" value="{{ old('stock', $ingredient->stock) }}" required
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none">
                    ...
                    <div>
                        ...
                        <select name="unit" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none bg-white">
                            ...
                            <option value="gram" {{ old('unit', $ingredient->unit) == 'gram' ? 'selected' : '' }}>Gram (gr)</option>
                            <option value="pcs" {{ old('unit', $ingredient->unit) == 'pcs' ? 'selected' : '' }}>Pcs (Buah)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="cost_per_unit" value="{{ old('cost_per_unit', $ingredient->cost_per_unit) }}" required
